...
export const getUserLikes = asyncHandler(async (req, res) => {
  ...
  if (postIds.length === 0) {
    return res.status(200).json(
      new ApiResponse(200, { likes: [], pagination: { currentPage: 1, totalPages: 0, totalLikes: 0 } }, "No likes found")
    );
  }
  ...
  return res.status(200).json(
    new ApiResponse(200, { likes: posts, pagination }, "User likes retrieved successfully")
  );
});
...
export const getPostLikes = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, { users, pagination }, "Post likes retrieved successfully")
  );
});
